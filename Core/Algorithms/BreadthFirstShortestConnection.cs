...
{
    /// <summary>
    ///这个类与BreadthFirstShortestPaths的区别就是不考虑连接的方向性。
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public class BreadthFirstShortestConnections<T> where T:IComparable<T>
    {
        ...
        /// </summary>
        public BreadthFirstShortestConnections(IGraph<T> Graph, T Source)
        {
            if (Graph == null)
        ...
        /// </summary>
        public BreadthFirstShortestConnections(IGraph<T> Graph, IList<T> Sources)
        {
            if (Graph == null)
                throw new ArgumentNullException();
        ...
        private void _breadthFirstSearch(IGraph<T> graph, T source)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                foreach (var adjacent in graph.AllNeighbours(current))
                {
