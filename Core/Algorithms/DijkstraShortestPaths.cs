...
using KRLab.Core.DataStructures.Heaps;
using System;
...
namespace KRLab.Core.Algorithms.Graphs
{
        ...
        where TVertex : IComparable<TVertex>
    {
        ...
        private void _dijkstra(TGraph graph, TVertex source)
        {
            ...
            while (!minPQ.IsEmpty)
            {
                ...
                foreach (var edge in edges)
                {
                    ...
                    var delta = Infinity;
                    if (edge.Weight.Weight < Infinity && Infinity - edge.Weight.Weight > _distances[currentIndex])  // Handles overflow
                    {
                        delta = _distances[currentIndex] + edge.Weight.Weight;
                    }
                    ...
                    {
                        _edgeTo[adjacentIndex] = edge as WeightedEdge<TVertex>;
        ...
        private bool _checkOptimalityConditions(TGraph graph, TVertex source)
        {
            ...
            foreach (var vertex in graph.Vertices)
            {
                ...
                foreach (var edge in graph.NeighboursMap(vertex))
                {
                    ...
                    if (_distances[v] + edge.Value.Weight < _distances[w])
                    {
            ...
            foreach (var vertex in graph.Vertices)
            {
                ...
                if (_distances[v] + edge.Weight.Weight != _distances[w])
                {
        ...
        public IEnumerable<TVertex> ShortestPathTo(TVertex destination)
        {
            ...
            var stack = new KRLab.Core.DataStructures.Lists.Stack<TVertex>();
