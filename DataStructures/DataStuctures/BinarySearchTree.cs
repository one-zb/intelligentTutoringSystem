...
namespace KRLab.Core.DataStructures.Trees
{
    /// <summary>
    ...
    public class BinarySearchTree<T> : IBinarySearchTree<T> where T : IComparable<T>
    {
        ...
        protected virtual void _replaceNodeInParent(BSTNode<T> node, BSTNode<T> newNode)//老师这里写成BSTNode<T> newNode=null,感觉不对
        {
            if (node.Parent != null)
            {
                if (node.IsLeftChild)
